...
+ (RACCompleteBlock)completeBlock:(RACSubject *)subject
{
    RACCompleteBlock completeBlock = ^(id output, NSError *error) {
        ...
        NSString *errorDescription = [output valueForKey:@"errorDescription"];
        NSString *errorCode = [output valueForKey:@"error"];
        if (error != nil ) {
            [subject sendError:error];
        } else if ( errorDescription != nil) {
            [subject sendError: [[NSError alloc] initWithDomain:@"10" code:errorCode.integerValue userInfo:[NSDictionary dictionaryWithObject:errorDescription                                                                      forKey:NSLocalizedDescriptionKey]]];
        } else {
