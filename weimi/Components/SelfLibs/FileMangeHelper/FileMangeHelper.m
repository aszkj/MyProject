...
- (void)clearDisk
{
    [[FileMangeHelper shareInstance] removeFileFromFileType:UploadFileAudioType];
    [[SDImageCache sharedImageCache] clearDisk];
    [[DatabaseCache shareInstance] DeleteAllByTableName:KTableNameOfSessionContent];
}

- (void)removeFileFromFileType:(UploadFileType)fileType
{
    NSString *removePath = [self getUploadPathFromFileType:fileType];
    //检查本地目录是否已存在
    NSFileManager *fileManager = [NSFileManager defaultManager];
    if ([fileManager fileExistsAtPath:removePath]) {
        BOOL removeResult = [fileManager removeItemAtPath:removePath error:nil];
        if (!removeResult) {
            DDLogError(@"删除%@失败",removePath);
        }
    }
}
