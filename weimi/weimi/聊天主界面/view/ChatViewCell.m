...
- (void)setVoiceImages {
    if (![_messageModel.msgType isEqualToString:@"SEND"]) {
...
-(void)setMessageModel:(SessonContentModel *)messageModel
{
    ...
    //判断信息是否发送成功
    if ([messageModel.msgType isEqualToString:@"SEND"])
    {
    ...
    if (messageModel.isMyself) {
        [WEMESimpleUser getSelfSimpleUserCompleteBlock:^(WEMESimpleUser *user) {
    ...
    switch (_dataModelType) {
        ...
        {
            [self setChantViewBackgroundImageWithRight:[messageModel.msgType isEqualToString:@"SEND"] withFrame:_cellFrame.backImageRect];
        ...
        case ProtoMessageContentTypeVoice: //语音
        {
            ...
            self.voiceTimeLabel.text = [NSString stringWithFormat:@"%ld\"",_messageModel.contentDuration/1000];//毫秒转秒
        ...
        case ProtoMessageContentTypeImage://图片
        {
            ...
            {
                [self.imageContent sd_setImageWithURL:TwiceImgUrl([NSURL URLWithString:messageModel.contentUrl], CGRectGetWidth(_cellFrame.imageRect), CGRectGetHeight(_cellFrame.imageRect)) placeholderImage:IMAGE(@"loaded")];
            }
            else
            {
...
{    
    if (IsEmpty(messageModel.key_id) && [messageModel.msgType isEqualToString:@"SEND"] && [_activityView isAnimating] && [messageModel.localId isEqualToString:self.messageModel.localId])
    {
        _errorButton.hidden = NO;
        [self ActivityAnimatinWithBOOL:NO];
...
- (void)sendSucceedChangeStatus:(BOOL)success
{
    if (success) {
        _errorButton.hidden = YES;
    } else {
...
-(UILabel *)timeLabel
{
    if (_timeLabel == nil) {
        ...
        _timeLabel.textColor = UIColorFromRGB(0Xadadad);
...
- (UIImageView *)imageContent {
    ...
    if (!_imageContent) {
        ...
        [_imageContent setContentMode:UIViewContentModeScaleAspectFill];
