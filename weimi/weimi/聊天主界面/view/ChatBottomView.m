...
//        _changeTextBtn = [XKO_CreateUIViewHelper createUIButtonBackNormalImage:IMAGE(@"keyboard") highlightedImage:nil radiusSize:0 frame:CGRectZero];
...
- (UILabel *)creatLabelWithText:(NSString *)text
{
    ...
    label.font = [UIFont customFontOfSize:13];
    return label;
}
...
//        [kNotification addObserver:self selector:@selector(textFieldDidChangeNotification) name:UITextFieldTextDidChangeNotification object:nil];
    }
...
- (RecordView *)recordView {
    ...
    if (!_recordView) {
        ...
        @weakify(self);
        [_recordView addTapActionWithBlock:^(UIGestureRecognizer *gestureRecoginzer) {
            @strongify(self);
            [self endAnimator];
...
{
    if (!button.isTouchInside) return;
