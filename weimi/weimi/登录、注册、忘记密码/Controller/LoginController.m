...
- (void)_creatUI{
    ...
    if ([kUserDefaults objectForKey:klastUsrAccount]) {
        _accountTextField.text = [kUserDefaults objectForKey:klastUsrAccount];
    }
    ...
    //快速注册，忘记密码
    self.quickRegisterButton= [XKO_CreateUIViewHelper createUIButtonWithTitle:@"新用户?立即注册" titleColor:chatStatus_Color titleFont:[UIFont customFontOfSize:14.0] backgroundColor:[UIColor whiteColor] hasRadius:YES targetSelector:@selector(quickRegister) target:self];
    [self.view addSubview:self.quickRegisterButton];
    
    self.fogetPasswdButton = [XKO_CreateUIViewHelper createUIButtonWithTitle:@"忘记密码?" titleColor:UIColorFromRGB(0x5c75a7) titleFont:[UIFont customFontOfSize:14] backgroundColor:kClearColor hasRadius:NO targetSelector:@selector(forgetPasswd) target:self];
...
- (void)_enterMainPage {
    ...
    [kUserDefaults setObject:self.accountTextField.text forKey:klastUsrAccount];
    ChatViewController *mainPage = [[ChatViewController alloc] init];
