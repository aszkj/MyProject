...
-(void)saveUserNickname
{
    ...
    WEAK_SELF
    _userControllerApi = [WEMEUsercontrollerApi sharedAPI];
    [_userControllerApi updateNickNameUsingPOST1WithCompletionBlock:_userNicknameTextField.text completionHandler:^(WEMESimpleResponse *output, NSError *error) {
    
        if (IsEmpty(error) && output.success.integerValue == YES)
        {
            if (_updateNickeNameBlock)
            {
                _updateNickeNameBlock(_userNicknameTextField.text);
