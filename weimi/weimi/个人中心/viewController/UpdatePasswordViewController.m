...
static CGFloat saveButtonHeight = 45.0f;
...
-(UIButton *)updateBtn
{
    if (!_updateBtn) {
        ...
        [_updateBtn setBackgroundImage:[[UIImage imageNamed:@"updateBack"] imageWithTintColor:[UINavigationBar appearance].barTintColor] forState:UIControlStateNormal];
        [_updateBtn setBackgroundImage:[[UIImage imageNamed:@"updateBack_select"] imageWithTintColor:[UINavigationBar appearance].barTintColor] forState:UIControlStateSelected];
...
{
    if (!_tableViewFooter) {
        _tableViewFooter = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, saveButtonHeight + 30)];
        ...
        [_updateBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.bottom.equalTo(@0);
//            make.left.equalTo(_tableViewFooter).with.offset(KLeftMargin);
//            make.right.equalTo(_tableViewFooter).with.offset(-KLeftMargin);
            make.centerX.equalTo(_tableViewFooter);
            make.width.equalTo(_tableViewFooter).with.offset(-2*KLeftMargin);
        }];
    }
