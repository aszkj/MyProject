...
NSString *const WEMEResponseObjectErrorKey = @"WEMEResponseObject";
...
@interface WEMEApiClient ()

@property (readwrite, nonatomic) NSDictionary *HTTPResponseHeaders;

@end

@implementation WEMEApiClient

- (instancetype)init {
    NSString *baseUrl = [[WEMEConfiguration sharedConfig] host];
...
+(NSNumber*) nextRequestId {
    @synchronized(self) {
        ...
        WEMEDebugLog(@"got id %ld", nextId);
        return [NSNumber numberWithLong:nextId];
    }
}

+(NSNumber*) queueRequest {
    NSNumber* requestId = [WEMEApiClient nextRequestId];
    WEMEDebugLog(@"added %@ to request queue", requestId);
...
-(Boolean) executeRequestWithId:(NSNumber*) requestId {
    ...
    if (matchingItems.count == 1) {
        WEMEDebugLog(@"removed request id %@", requestId);
        [queuedRequests removeObject:requestId];
        return YES;
    }
    else {
    ...
    [self.reachabilityManager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        ...
        switch (status) {
            case AFNetworkReachabilityStatusUnknown:
                WEMEDebugLog(@"reachability changed to AFNetworkReachabilityStatusUnknown");
                [WEMEApiClient setOfflineState:true];
                break;

            case AFNetworkReachabilityStatusNotReachable:
                WEMEDebugLog(@"reachability changed to AFNetworkReachabilityStatusNotReachable");
                [WEMEApiClient setOfflineState:true];
                break;

            case AFNetworkReachabilityStatusReachableViaWWAN:
                WEMEDebugLog(@"reachability changed to AFNetworkReachabilityStatusReachableViaWWAN");
                [WEMEApiClient setOfflineState:false];
                break;

            case AFNetworkReachabilityStatusReachableViaWiFi:
                WEMEDebugLog(@"reachability changed to AFNetworkReachabilityStatusReachableViaWiFi");
                [WEMEApiClient setOfflineState:false];
                break;
            default:
                break;
...
//            return [NSString stringWithString:data];
            ...
            return err?@"":[[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding];
                                                               ...
                                                               success:^(AFHTTPRequestOperation *operation, id response) {
                                                                   ...
                                                                   if ([self executeRequestWithId:requestId]) {
                                                                       ...
                                                                       if (operation.responseObject) {
                                                                           ...
                                                                           userInfo[WEMEResponseObjectErrorKey] = operation.responseObject;
                                                                       }
...
-(NSNumber*)  requestWithCompletionBlock: (NSString*) path
    ...
    if ([requestContentType isEqualToString:@"application/json"]) {
        self.requestSerializer = [WEMEJSONRequestSerializer serializer];
    }
     ...
     WEMEConfiguration *config = [WEMEConfiguration sharedConfig];
    [self.requestSerializer setAuthorizationHeaderFieldWithCredential:config.credential];
     // setting response serializer
    if ([responseContentType isEqualToString:@"application/json"]) {
        self.responseSerializer = [WEMEJSONResponseSerializer serializer];
    }
    else {
    ...
    [pathParams enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        [resourcePath replaceCharactersInRange:[resourcePath rangeOfString:[NSString stringWithFormat:@"%@%@%@", @"{", key, @"}"]]
                                    withString:[WEMEApiClient escape:obj]];
    }];
    ...
    if (offlineState) {
        WEMEDebugLog(@"%@ cache forced", resourcePath);
        [request setCachePolicy:NSURLRequestReturnCacheDataDontLoad];
    }
    else if(!hasHeaderParams && [method isEqualToString:@"GET"] && cacheEnabled) {
        WEMEDebugLog(@"%@ cache enabled", resourcePath);
        [request setCachePolicy:NSURLRequestUseProtocolCachePolicy];
    }
    else {
        WEMEDebugLog(@"%@ cache disabled", resourcePath);
    ...
    NSNumber* requestId = [WEMEApiClient queueRequest];
        ...
        for(NSString * key in [queryParams keyEnumerator]){
            ...
            if ([queryParam isKindOfClass:[NSString class]]){
                [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator,
                                          [WEMEApiClient escape:key], [WEMEApiClient escape:[queryParams valueForKey:key]]]];
            }
            else if ([queryParam isKindOfClass:[WEMEQueryParamCollection class]]){
                WEMEQueryParamCollection * coll = (WEMEQueryParamCollection*) queryParam;
                NSArray* values = [coll values];
                NSString* format = [coll format];

                if ([format isEqualToString:@"csv"]) {
                    [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator,
                        [WEMEApiClient escape:key], [NSString stringWithFormat:@"%@", [values componentsJoinedByString:@","]]]];
                    ...
                    for(id obj in values) {
                        [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator,
                            [WEMEApiClient escape:key], [NSString stringWithFormat:@"%@", obj]]];
                        counter += 1;
                    }
                ...
                [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator,
                                          [WEMEApiClient escape:key], [NSString stringWithFormat:@"%@", [queryParams valueForKey:key]]]];
            }
...
- (id) sanitizeForSerialization:(id) object {
    ...
    else if ([object isKindOfClass:[NSString class]] || [object isKindOfClass:[NSNumber class]] || [object isKindOfClass:[WEMEQueryParamCollection class]]) {
        return object;
    }
    ...
    else if ([object isKindOfClass:[WEMEObject class]]) {
