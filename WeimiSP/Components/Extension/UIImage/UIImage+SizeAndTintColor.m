...
//+(void)uploadImageWithUIImage:(UIImage *)selectedImg succeedBlock:(StartIFlySpeechBlock)block channel:(NSString *)channel
//{
...
//    
//    if (block) {
//        block(@"图片",0,UploadFileImageType,nil,[fileName lastPathComponent],imgRect);
//    }
//    CGFloat scale = selectedImg.scale;
//    
//    [[WEMEFileuploadcontrollerApi sharedAPI] uploadFileUsingPOSTWithCompletionBlock:channel fileType:@"IMAGE" file:[NSURL fileURLWithPath:fileName] completionHandler:^(NSString *output, NSError *error) {
//        
//        if (!IsEmpty(output)) {
//            
...
//                
//                if (block) {
//                    block(@"图片",0,UploadFileImageType,resultDic[@"url"],fileName, CGSizeMake(imgRect.width * scale, imgRect.height * scale));
//                }
//            }
//        }
//    }];
