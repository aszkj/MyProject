...
- (UIView *)line {
    ...
    if (!_line) {
        ...
        CGFloat height = kScreenHeight/2 - (_popverView.bounds.size.height/2 - 16);
        _line = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 12, height)];
        _line.backgroundColor = [UIColor clearColor];
        
        UIView *lineBgView = [XKO_CreateUIViewHelper createUIViewWithBackgroundColor:kGetColor(100, 100, 101)];
        lineBgView.layer.cornerRadius = 6;
        lineBgView.layer.masksToBounds = YES;
        [_line addSubview:lineBgView];
        
        [lineBgView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.centerY.equalTo(_line.mas_bottom);
            make.centerX.equalTo(_line);
            make.size.mas_equalTo(CGSizeMake(12, 12));
        }];
        
        UIView *topLine = [[UIView alloc] initWithFrame:CGRectZero];
        topLine.backgroundColor = UIColorFromRGB(0Xd4dde8);
        [_line addSubview:topLine];
        
        [topLine mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(@0);
            make.bottom.equalTo(lineBgView.mas_centerY);
...
//        _popverView.transform = CGAffineTransformMakeRotation(M_PI * 0.01);
        ...
        self.line.center = CGPointMake(_popverView.center.x, self.line.bounds.size.height/2);
...
- (void)fadeOut
{
    [UIView animateWithDuration:.35 animations:^{
        if (self.popverViewHeight && self.popverViewHeight > 0) {
            _popverView.center = CGPointMake(kScreenWidth / 2, -_popverView.frame.size.height/2);
        }
        else {
            _popverView.center = CGPointMake(kScreenWidth / 2, -_popverView.frame.size.height/2);
        }
...
- (void)show
{
    ...
    [kCurrentKeyWindow addSubview:self.line];
