...
- (void)initValues
{
    ...
    bottomLine.frame = CGRectMake(0, kHeightOfTopScrollView, self.bounds.size.width, 0.5);
    bottomLine.backgroundColor = [UIColor blackColor];
    [self addSubview:bottomLine];

    //创建主滚动视图

    _rootScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, kHeightOfTopScrollView + 0.5, self.bounds.size.width, self.bounds.size.height - kHeightOfTopScrollView - 0.5)];
...
- (void)createNameButtons
{
    ...
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        if (i == 0) {
            ...
            _shadowImageView.frame = CGRectMake(48, kHeightOfTopScrollView - kHeightOfShadowImage, self.bounds.size.width/[_viewArray count] - 96, kHeightOfShadowImage);
            button.selected = YES;
        }
...
- (void)selectNameButton:(UIButton *)sender
{
    ...
    if (!sender.selected) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            [_shadowImageView setFrame:CGRectMake(sender.frame.origin.x + 48, kHeightOfTopScrollView - kHeightOfShadowImage, sender.frame.size.width - 96, kHeightOfShadowImage)];
