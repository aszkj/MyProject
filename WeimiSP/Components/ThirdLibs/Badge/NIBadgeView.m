...
- (void)drawRect:(CGRect)rect {
  ...
  const CGFloat badgeRadius = textSize.height / 2.f + (sUsesSolidTint ? 0.5f : 0);
  ...
  CGContextAddArc(context, maxX-badgeRadius, minY+badgeRadius, badgeRadius, pi+(pi/2), 0, 0);
  CGContextAddArc(context, maxX-badgeRadius, maxY-badgeRadius, badgeRadius, 0, pi/2, 0);
  CGContextAddArc(context, minX+badgeRadius, maxY-badgeRadius, badgeRadius, pi/2, pi, 0);
  CGContextAddArc(context, minX+badgeRadius, minY+badgeRadius, badgeRadius, pi, pi+pi/2, 0);
  if (self.shadowColor) {
  ...
  if (!sUsesSolidTint) {
    ...
    CGContextAddArc(context, maxX-badgeRadius, minY+badgeRadius, badgeRadius, pi+(pi/2), 0, 0);
    CGContextAddArc(context, minX+badgeRadius, minY+badgeRadius, badgeRadius, pi, pi+pi/2, 0);
    CGContextAddRect(context, CGRectMake(minX, minY + badgeRadius,
                                         rect.size.width - badgeRadius + 1, CGRectGetMidY(rect) - badgeRadius));
