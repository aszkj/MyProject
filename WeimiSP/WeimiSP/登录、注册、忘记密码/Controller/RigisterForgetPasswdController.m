...
- (void)_init {
    ...
    self.navigationController.navigationBar.barTintColor = UIColorFromRGB(0x12b7f5);
...
- (void)_createUI {
    ...
    for (NSInteger i=0; i<textArr.count; i++) {
        ...
        if ([textArr[i] isEqualToString:@"请输入验证码"]) {
            ...
            //添加获取验证码button
            UIButton *getVeryCodeButton = [XKO_CreateUIViewHelper createUIButtonWithTitle:@"获取验证码" titleColor:kGetColor(217, 217, 221) titleFont:kSystemFontWithSize(11) backgroundColor:kClearColor hasRadius:YES targetSelector:@selector(getVeryCode:) target:self];
    ...
    self.registerButton.backgroundColor = UIColorFromRGB(0x12b7f5);
