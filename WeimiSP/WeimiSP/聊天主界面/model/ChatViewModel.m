...
- (RACSignal *)initializeDataSource {
    ...
    NSArray<SessonContentModel *> *list = [[DatabaseCache shareInstance] selectPageDataFromTimestamp:[self.firstupdate stringValue] channel:self.channel size:10 withTablename:KTableNameOfSessionContent];
...
- (RACSignal *)getUnpullData:(BOOL)searchLast {
    ...
    if (searchLast) {
        ...
        [[WEMETaskcontrollerApi sharedAPI] getTaskMessageUsingGETWithCompletionBlock:[self lastupdate] page:@(0) datesearch:self.datesearch completionHandler:^(WEMEPageResponseOfMongoMessage *output, NSError *error) {
        ...
        //收索历史记录
        self.datesearch = @1;
        [[WEMETaskcontrollerApi sharedAPI] getTaskMessageUsingGETWithCompletionBlock:[self firstupdate] page:@(0) datesearch:self.datesearch completionHandler:^(WEMEPageResponseOfMongoMessage *output, NSError *error) {
            completeBlock(output,error);
        }];
