...
- (CGRect)getTextRect:(NSString *)string {
    ...
    CGFloat height = optimumSize.height > 17 ? optimumSize.height:17;
    width = width < 25 ? 25 :width;
    if (self.messageModel.isMyself)//右边
    {
...
//            CGFloat width = CGRectGetMinX(self.headImageRect) - x -  kSpacing;
        ...
        {
            _backImageRect = CGRectMake(CGRectGetMaxX(self.headImageRect) + avatorToBackImage, CGRectGetMinY(self.textRect) - kBackImageEdge , CGRectGetWidth(self.textRect) + 2 * kBackImageEdge + kBackImageEdge / 5, CGRectGetHeight(self.textRect) + 2 * kBackImageEdge);
        }
    }
