...
//    _twoLine.backgroundColor = lineColor;
    ...
    [self setupButton:_decreaseBtn normalImage:@"jian" HighlightImage:nil];
    [self addSubview:_decreaseBtn];
    
    _increaseBtn = [[UIButton alloc] init];
    [self setupButton:_increaseBtn normalImage:@"nojia" HighlightImage:nil];
...
//    _textField.rightView = view;
    ...
    _textField.font = [UIFont fontWithName:@"STHeitiSC-Medium" size:13];
    [_textField addTarget:self action:@selector(textFieldChanged:) forControlEvents:UIControlEventEditingChanged];
    [_textField setTextColor:UIColorFromRGB(0x333333)];
...
- (void)increase{
    ...
    if (newNum==_maxResult) {
        [_increaseBtn setImage:[UIImage imageNamed:@"nojia"] forState:UIControlStateNormal];
    }
}
...
- (void)decrease{
    ...
    if (newNum >= _smallResult) {
         [_increaseBtn setImage:[UIImage imageNamed:@"jia"] forState:UIControlStateNormal];
        _textField.text = [NSString stringWithFormat:@"%i", newNum];
        self.callBack(_textField.text);
    } else {
