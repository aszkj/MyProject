...
- (void)setup
{
    ...
    _tagTextFont = [UIFont systemFontOfSize:12.0f];
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CGSize maxSize = CGSizeMake(collectionView.frame.size.width - layout.sectionInset.left - layout.sectionInset.right, self.itemHeight);
    CommonCategaryModel *model = self.tags[indexPath.item];
    NSString *categaryName = model.categaryName;
    CGRect frame = [categaryName boundingRectWithSize:maxSize options:NSStringDrawingUsesFontLeading|NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: self.tagTextFont} context:nil];
    
    return CGSizeMake(frame.size.width + 20.0f, self.itemHeight);
}
