...
public class RefreshLayout extends ViewGroup {
    ...
    public static final int HEADER_VIEW_HEIGHT = 50;// HeaderView height (dp)
    ...
    // SwipeRefreshLayout内的目标View，比如RecyclerView,ListView,ScrollView,GridView
    // etc.
    ...
    private boolean handlerPushTouchEvent(MotionEvent ev, int action) {
        switch (action) {
            ...
            case MotionEvent.ACTION_CANCEL: {
                ...
                mIsBeingDragged = false;
                mActivePointerId = INVALID_POINTER;
                if (overscrollBottom < mFooterViewHeight) {// 直接取消
                    pushDistance = 0;
    ...
    private OnDataRefreshListener refreshListener;
    private OnDataLoadListener loadListener;

    public void init(Context context) {
        initHeaderRefreshView(context);
        initFooterRefreshView(context);
        setHeaderView(header);
        setFooterView(footer);
    }

    private void initHeaderRefreshView(Context context) {
        header = (ViewGroup) LayoutInflater.from(context).inflate(R.layout.refresh_header, null);
    ...
    public void onPullEnable(boolean enable) {
        headerText.setText(enable ? R.string.pull_to_refresh_release_label : R.string.pull_to_refresh_pull_label);
        headerImage.setImageResource(enable ? R.mipmap.pulltorefresh_up_arrow : R.mipmap.pulltorefresh_down_arrow);
        headerImage.setVisibility(VISIBLE);
        headerProgress.setVisibility(View.GONE);
    }

    private void initFooterRefreshView(Context context) {
        footer = (ViewGroup) LayoutInflater.from(context).inflate(R.layout.refresh_footer, null);
        ...
        footerImage.setImageResource(R.mipmap.pulltorefresh_up_arrow);
    }

    public void onLoadMore() {
        footerText.setText(R.string.pull_to_refresh_refreshing_label);
        footerImage.setVisibility(GONE);
        footerProgress.setVisibility(VISIBLE);
        if (loadListener != null) {
            loadListener.onDataLoad();
        }
    }
    ...
    public void onPushEnable(boolean enable) {
        footerText.setText(enable ? R.string.pullup_to_release : R.string.pullup_to_refresh);
        footerImage.setImageResource(enable ? R.mipmap.pulltorefresh_down_arrow : R.mipmap.pulltorefresh_up_arrow);
        footerImage.setVisibility(VISIBLE);
        footerProgress.setVisibility(View.GONE);
    }

    public void setRefreshListener(OnDataRefreshListener refreshListener) {
