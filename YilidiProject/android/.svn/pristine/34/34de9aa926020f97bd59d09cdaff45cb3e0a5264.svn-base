...
public class VerticalViewPager extends ViewGroup {
     ...
     * the scroll position is changed. This allows the application to apply custom property transformations to each page,
     * overriding the default sliding look and feel.
     * <p/>
     * <p>
     * <em>Note:</em> Prior to Android 3.0 the property animation APIs did not exist. As a result, setting a PageTransformer
     * prior to Android 3.0 (API 11) will have no effect.
     * </p>
     *
     * @param reverseDrawingOrder true if the supplied PageTransformer requires page views to be drawn from last to first instead of first to
     *                            last.
    ...
    /**
     * Returns the number of pages that will be retained to either side of the current page in the view hierarchy in an idle
     * state. Defaults to 1.
     *
    ...
    /**
     * Set the number of pages that should be retained to either side of the current page in the view hierarchy in an idle
     * state. Pages beyond this limit will be recreated from the adapter when needed.
     * <p/>
     * <p>
     * This is offered as an optimization. If you know in advance the number of pages you will need to support or have
     * lazy-loading mechanisms in place on your pages, tweaking this setting can have benefits in perceived smoothness of
     * paging animations and interaction. If you have a small number of pages (3-4) that you can keep active all at once,
    ...
    void populate(int newCurrentItem) {
        ...
        if (curItem != null) {
            ...
            final float topHeightNeeded = clientHeight <= 0 ? 0 : 2.f - curItem.heightFactor + (float) getPaddingLeft()
                    / (float) clientHeight;
            ...
            if (extraHeightBottom < 2.f) {
                ...
                final float bottomHeightNeeded = clientHeight <= 0 ? 0 : (float) getPaddingRight() / (float) clientHeight
                        + 2.f;
    ...
    /**
     * This is the persistent state that is saved by ViewPager. Only needed if you are creating a sublass of ViewPager that
     * must save its own state, in which case it should implement a subclass of this which contains that state.
     */
            ...
            if (child.getVisibility() != GONE) {
                ...
                if (!lp.isDecor && (ii = infoForChild(child)) != null) {
                    ...
                    if (lp.needsMeasure) {
                        ...
                        final int widthSpec = MeasureSpec.makeMeasureSpec((int) (width - paddingLeft - paddingRight),
                                MeasureSpec.EXACTLY);
     ...
     * scroll or a user initiated touch scroll. If you override this method you must call through to the superclass
     * implementation (e.g. super.onPageScrolled(position, offset, offsetPixels)) before onPageScrolled returns.
     *
    ...
    /**
     * @return Info about the page at the current scroll position. This can be synthetic for a missing middle page; the
     * 'object' field can be null.
     */
    ...
    private int determineTargetPage(int currentPage, float pageOffset, int velocity, int deltaY) {
        int targetPage;
        if (Math.abs(deltaY) > 0 && Math.abs(velocity) > 0) {
     ...
     * <p/>
     * <p/>
     * A fake drag can be useful if you want to synchronize the motion of the ViewPager with the touch scrolling of another
     * view, while still letting the ViewPager control the snapping motion and fling behavior. (e.g. parallax-scrolling
     * tabs.) Call {@link #fakeDragBy(float)} to simulate the actual drag motion. Call {@link #endFakeDrag()} to complete the
     * fake drag and fling as necessary.
     * <p/>
     * <p/>
     * During a fake drag the ViewPager will ignore all touch events. If a real drag is already in progress, this method will
     * return false.
     *
    ...
    protected boolean canScroll(View v, boolean checkV, int dy, int x, int y) {
        ...
        if (v instanceof ViewGroup) {
            ...
            for (int i = count - 1; i >= 0; i--) {
                ...
                if (y + scrollY >= child.getTop() && y + scrollY < child.getBottom() && x + scrollX >= child.getLeft()
    ...
    /**
     * You can call this function yourself to have the scroll view perform scrolling from a key event, just as if the event
     * had been dispatched to it by the view hierarchy.
     *
    ...
    public static class LayoutParams extends ViewGroup.LayoutParams {
        ...
        /**
         * Gravity setting for use on decor views only: Where to position the view page within the overall ViewPager
