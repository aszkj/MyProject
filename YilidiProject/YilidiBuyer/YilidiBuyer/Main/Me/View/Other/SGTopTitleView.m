...
- (void)setStaticTitleArr:(NSArray *)staticTitleArr {
    ...
    for (NSInteger j = 0; j < self.staticTitleArr.count; j++) {
        ...
        _staticTitleLabel.textColor = KTextColor;
        _staticTitleLabel.font = [UIFont systemFontOfSize:14];
        // 设置高亮文字颜色
        _staticTitleLabel.highlightedTextColor = KCOLOR_PROJECT_RED;
        ...
        _line.frame = CGRectMake(labelX, self.frame.size.height/3, 0.5, labelH/2-5);
        _line.backgroundColor = UIColorFromRGB(0xe6e6e6);
    ...
    _indicatorView.backgroundColor = KCOLOR_PROJECT_RED;
...
- (void)seletedIndex:(NSInteger)index{
    ...
    UILabel *selLabel =  [self.allTitleLabel objectAtIndex:index];
     [self staticTitleLabelSelecteded:selLabel];
}
...
- (void)staticTitleLabelSelecteded:(UILabel *)label {
    ...
    // 颜色恢复
    _selectedTitleLabel.textColor = KTextColor;
...
- (void)setScrollTitleArr:(NSArray *)scrollTitleArr {
    ...
    for (NSUInteger i = 0; i < self.scrollTitleArr.count; i++) {
        ...
        // 设置高亮文字颜色
        _scrollTitleLabel.highlightedTextColor = KCOLOR_PROJECT_RED;
...
//        label.highlightedTextColor = titleAndIndicatorColor;
        //选中文字颜色
        label.highlightedTextColor = KTextColor;
    }
