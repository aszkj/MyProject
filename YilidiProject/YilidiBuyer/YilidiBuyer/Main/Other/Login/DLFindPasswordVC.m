...
- (void)_postData {
    ...
    if (_resetPassView.ConfirmPassWord.text.length==0) {
        ...
        [self showSimplyAlertWithTitle:@"提示" message:@"请输入确认密码" sureAction:^(UIAlertAction *action) {
    ...
    if (![_resetPassView.setPassWord.text isEqualToString:_resetPassView.ConfirmPassWord.text]){
        [self showSimplyAlertWithTitle:@"提示" message:@"新密码与确认密码输入不一致" sureAction:^(UIAlertAction *action) {
    ...
    [AFNHttpRequestOPManager postWithParameters:parameters subUrl:kUrl_ForgotPassword block:^(NSDictionary *resultDic, NSError *error) {
        if (error.code==1) {
            [self hideLoadingHub];
            [self hideHubWithOnlyText:@"修改密码成功"];
            
            [self performSelector:@selector(_goback) withObject:nil afterDelay:2];
...
-(BOOL)isCheckContent:(NSString *)text{
    ...
    if (_editorStatus==TextEditorPhoneFieldTag) {
        if (text.length == 11) {
            ...
            [_validationButton setTitleColor:KTextColor forState:UIControlStateNormal];
            [_validationButton setBackgroundColor:KLineColor];
        ...
        if (_resetPassView.setPassWord.text.length>=5&&_resetPassView.ConfirmPassWord.text.length>=5) {
            ...
            [_resetPassView.submitButton setTitleColor:KTextColor forState:UIControlStateNormal];
            [_resetPassView.submitButton setBackgroundColor:KLineColor];
