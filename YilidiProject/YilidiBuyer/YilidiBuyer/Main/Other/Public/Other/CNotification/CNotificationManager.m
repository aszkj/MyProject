...
#define kNOTIFICATION_VIEW_HEIGHT 130

@interface CNotificationManager()

@property (nonatomic,readonly,getter=isShowing) BOOL showing;
@property (nonatomic,strong) UILabel *titleLabel;
@property (nonatomic,strong) RemoteNotificationView *backgroundView;
@property (nonatomic,assign) BOOL userHasClicked;
...
//        [self.backgroundView addSubview:self.titleLabel];
        ...
        UITapGestureRecognizer *dismissTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(clickToDissmiss)];
...
+ (void)showMessage:(PushMessageModel *)message {
    [CNotificationManager showMessage:message withOptions:nil completeBlock:nil];

}
+ (void)showMessage:(PushMessageModel *)message withOptions:(NSDictionary *)options {
    [CNotificationManager showMessage:message withOptions:options completeBlock:nil];

}
+ (void)showMessage:(PushMessageModel *)message withOptions:(NSDictionary *)options completeBlock:(CNCompleteBlock)completeBlock{
 ...
 */
- (void)setupViewOptionsWithMessage:(PushMessageModel *)message{
//    self.backgroundView.backgroundColor = self.backgroundColor;
    self.titleLabel.textColor = self.textColor;
    self.titleLabel.font = self.textFont;
//    self.titleLabel.text = message;
    self.backgroundView.messageModel = message;
}
 ...
 */
- (void)showNotification:(PushMessageModel  *)message{
 ...
 */
- (void)reDisplayTitleLabel:(PushMessageModel *)message{
...
- (void)clickToDissmiss {
    self.userHasClicked = YES;
...
- (void)dismissNotification{
    ...
    [UIView animateWithDuration:.5 animations:^{
        ...
        if (self.completeBlock) {
            self.completeBlock(self.userHasClicked);
            self.userHasClicked = NO;
...
- (RemoteNotificationView *)backgroundView{
    if (!_backgroundView) {
//        _backgroundView = [[UIView alloc] initWithFrame:CGRectMake(0, -kNOTIFICATION_VIEW_HEIGHT, [UIScreen mainScreen].applicationFrame.size.width, kNOTIFICATION_VIEW_HEIGHT)];
        _backgroundView = BoundNibView(@"RemoteNotificationView", RemoteNotificationView);
        _backgroundView.frame = CGRectMake(0, -kNOTIFICATION_VIEW_HEIGHT, [UIScreen mainScreen].applicationFrame.size.width, kNOTIFICATION_VIEW_HEIGHT);
