...
//    [self.progressView py_setProgress:photo.progress animated:NO];
    ...
    [self sd_setImageWithURL:url placeholderImage:placeholdeerImage options:0 progress:^(NSInteger receivedSize, NSInteger expectedSize) {
        if (weak_self.isBig) {
            // 获取图片链接
            NSString *imageUrl = [[weak_self sd_imageURL] absoluteString];
            for (PYPhoto *photo in weak_self.photosView.photos) {
            ...
            if ([imageUrl isEqualToString:weak_self.photo.original_pic] || (!weak_self.photo.original_pic &&
                [imageUrl isEqualToString:weak_self.photo.thumbnail_pic])) { // 图片为当前PYPhotoView的图片
//                weak_self.progressView.hidden = NO;
//                [weak_self.progressView py_setProgress:weak_self.photo.progress animated:YES];
            }
        }
    } completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
//        weak_self.progressView.hidden = YES;
        if (image && ([[imageURL absoluteString] isEqualToString:weak_self.photo.thumbnail_pic] ||
                      [[imageURL absoluteString] isEqualToString:weak_self.photo.original_pic])) { // 图片为当前PYPhotoView的图片并且不是占位图（占位图 image会为null）
            // 允许手势
            [weak_self addGestureRecognizers];
            // 记录原始大小
            weak_self.photo.originalSize = CGSizeMake(weak_self.py_width, weak_self.py_width * image.size.height / image.size.width);
            // 记录未旋转的宽度或者旋转完成时的宽度
            weak_self.photo.verticalWidth = weak_self.photo.originalSize.width;

            if (!weak_self.isBig) {
                weak_self.photo.thumbnailImage = image;
            } else {
                weak_self.photo.originalImage = image;
                weak_self.photo.progress = 1.0;
            }
        }
        // 图片加载失败(是否隐藏)
