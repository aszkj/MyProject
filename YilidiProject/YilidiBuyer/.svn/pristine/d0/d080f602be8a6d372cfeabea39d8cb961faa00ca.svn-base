...
- (void)setSearchKeyWords:(NSString *)searchKeyWords {
    self.searchTextField.text = searchKeyWords;
}
...
//    self.cancelOrSearchButton.selected = NO;
    [self.searchTextField setTextPlaceHolderFont:kSystemFontSize(14.0f) placeHolderTextColor:KTextColor];
...
//    }
//    
//    if (![string sui_validateCommonInputTwo]) {
//        return NO;
//    }
//    
//    return YES;
...
- (IBAction)clearTextFieldAction:(id)sender {
    ...
    self.keyWords = nil;
    emptyBlock(self.clearButtonBlock);
}
