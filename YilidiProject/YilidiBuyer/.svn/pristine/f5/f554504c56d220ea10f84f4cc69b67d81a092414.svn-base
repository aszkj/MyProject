...
//    [self _cancelListenAppEnterForground];
}
    ...
    [self.userLocationManager startUserLocationBackBlock:^(CLLocation *location, NSString *city) {
        if (weak_self.hasSuccessLocated) {
            ...
            if (latitude && lontitude) {
                ...
                [self _configureLongtitude:lontitude latitude:latitude];
...
//        [self dissmiss];
        [self _configureLongtitude:kDefaultSeaLongtitude latitude:kDefaultSeaLatitude];
        [self _requestNearbyDefaultShopWithLogintitude:kDefaultSeaLongtitude latitude:kDefaultSeaLatitude];

    }];
}

- (void)_configureLongtitude:(CGFloat)longtitude latitude:(CGFloat)latitude{
    [kUserDefaults setObject:@(longtitude) forKey:KLongtitudeKey];
    [kUserDefaults setObject:@(latitude) forKey:KLatitudeKey];
    self.locatedLatitude = latitude;
    self.locatedLongtitude = longtitude;
}
...
//         weak_self.homeTopView.bgView.backgroundColor=[barColor colorWithAlphaComponent:contentOffset / 200];
        BOOL communityUIShow = contentOffset <= 80;
        [weak_self.homeTopView displayCommunityUI:communityUIShow];
...
- (void)_registerNotofication {
    ...
    [kNotification addObserver:self selector:@selector(observeToFloorGoodsPage:) name:KNotificationLookFloorGoods object:nil];
...
- (void)_addShopCartAction:(NSNotification *)info{
    ...
    [DLAnimatePerformer excuteAddShopCartAnimationUsingAnimateImageView:willAnimateImgView contentListView:self.homeFoodCollectionView shopCartViewPoint:MainShopCartIconPoint isAddShopCart:isAdd];
}

- (void)handleRemotePushMessage {
    if ([[PushMessageManager sharedManager] hasLauchRemotePushMessage]) {
        if (_storeCloseShowView) {
            _storeCloseShowView.hidden = YES;
        }
        if (_homeGotRedPacketView) {
            _homeGotRedPacketView.hidden = YES;
        }
        [[PushMessageManager sharedManager] handleAppLaunchMessageJumpLogic];
    }
}
...
- (void)_requestHeaderData {
    [self _requestHomeIconData];
}

- (void)_requestHomeIconData {
    [AFNHttpRequestOPManager getInfoWithSubUrl:kUrl_GetHomeIconUrlList parameters:nil block:^(id result, NSError *error) {
        ...
        resultArr = [resultArr sui_map:^NSString *(NSDictionary *obj, NSUInteger index) {
            return obj[@"imageUrl"];
        }];
        self.homeFoodCollectionView.homeFirstSectionView.homeIconArr = resultArr;
        [self _requetHeaderLoopData];
...
- (void)_requestHolidayAdData {
    ...
    [AFNHttpRequestOPManager postWithParameters:self.requestParam subUrl:kUrl_HomeHeaderLoopPlayUrl block:^(NSDictionary *resultDic, NSError *error) {
        ...
        [self.homeFoodCollectionView.homeFirstSectionView setHeaderHolidayAdArr:resultArr getHolidayAdHeightBlock:^(CGFloat holidayAdHeight) {
            if (holidayAdHeight > 0) {
                self.homeFoodCollectionView.headerHeight = self.homeFoodCollectionView.headerHeight-kHolidayAdPartHeight+holidayAdHeight;
            }
            [self.homeFoodCollectionView reloadData];
        }];
    }];
...
//            return ;
        ...
        if (![self _judgeRelevanceStoreOfCommunity:communityModel]) {
            ...
            [self _checkExceptionCondition];
        }
        [self _configureHomePageData];
//        [self performSelector:@selector(handleRemotePushMessage) withObject:nil afterDelay:3.0f];
        [self handleRemotePushMessage];
...
- (HomeGotErrorView *)homeGotErrorView {
    if (!_homeGotErrorView) {
        ...
        [_homeGotErrorView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.mas_equalTo(kDLhomeHeaderCycleViewHeight-20);
