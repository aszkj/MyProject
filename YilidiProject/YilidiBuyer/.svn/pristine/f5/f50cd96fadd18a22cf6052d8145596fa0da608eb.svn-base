...
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *secKilAdImgViewWidthToScreenAspeciaConstraint;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *secKillAdImgViewWidthConstraint;
...
-(void)_init {
    ...
    [self performSelector:@selector(_setHeaderPlaceHolderImage) withObject:nil afterDelay:0.3];
}
...
- (void)_initSeckillActivity {
    ...
    self.seckillActivityViewWidthConstraint.constant = KSeckillPartViewWidth;
    self.seckillActivityHeightConstraint.constant = KSeckillPartViewHeight;
    
    CGFloat secKilAdImgViewWidthToScreenAspecia = 0.20;
    CGFloat seckillAdImgViewToTopConstraint = 20;
    if (iPhone5 || iPhone4) {
        secKilAdImgViewWidthToScreenAspecia = 0.18;
        seckillAdImgViewToTopConstraint = 15;
    }
    self.secKillAdImgViewWidthConstraint.constant = kScreenWidth * secKilAdImgViewWidthToScreenAspecia;
    self.seckillActivityImgView.topConstraint.constant = seckillAdImgViewToTopConstraint;
...
- (void)_setHeaderPlaceHolderImage {
    self.headerCycleView.placeholderImage = IMAGE(@"placeHolder_rectangle");
}
...
- (void)_initSpecialItemView {
    ...
    [self.specialItemCollectionView configureCollectionCellNibName:@"DLHomeSpecialItemCollectionViewCell" configureCollectionCellData:^(UICollectionView *collectionView,id collectionModel, UICollectionViewCell *collectionCell,NSIndexPath *clickIndexPath) {
        ...
        [cell.itemIconImgView sd_SetImgWithUrlStr:model.iconImgUrl placeHolderImgName:model.iconName options:SDWebImageRefreshCached];
...
//            [kNotification postNotificationName:KNotificationLookGoodsClassPage object:defineOneKeyValueDic(kLinkDataKeyGoodsClass, TOP_CLASS_CODE) userInfo:nil];
            [Util ShowAlertWithOnlyMessage:FunctionTemporarilyNotOpenAlert];
            
        }
    }];
    self.specialItemCollectionView.commonFlowLayout.itemSize = CGSizeMake(kScreenWidth / 4.0, 70);
...
-(void)_initSpecialSbjectCollectionView {
    ...
    WEAK_SELF
    [self.specialSubjectCollectionView configureCollectionCellNibName:@"DLHomeSpecialSubjectCell" configureCollectionCellData:^(UICollectionView *collectionView,id collectionModel, UICollectionViewCell *collectionCell,NSIndexPath *clickIndexPath) {
    ...
    self.specialSubjectCollectionView.commonFlowLayout.itemSize = CGSizeMake(kSpecaiSubjectSecondlPartWidth, kSpecaiSubjectSecondlPartHeight);
    self.specialItemCollectionView.commonFlowLayout.minimumInteritemSpacing = 0;
    self.specialItemCollectionView.commonFlowLayout.minimumLineSpacing = 0;
    self.specialSubjectCollectionView.commonFlowLayout.scrollDirection = UICollectionViewScrollDirectionHorizontal;
    self.specialItemCollectionView.commonFlowLayout.sectionInset = UIEdgeInsetsMake(0, 0, 0, 0);
      ...
      getHolidayAdHeightBlock:(void(^)(CGFloat holidayAdHeight))getHolidayAdHeightBlock
{
    ...
    if (headerHolidayAdArr.count) {
        ...
        WEAK_SELF
        [self.holidayAdImgView sd_setImageWithURL:[NSURL URLWithString:firstHolidayAdModel.imgUrl] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
            if (!image) {
                return;
            }
            CGSize size = image.size;
            CGFloat imgAspecio = size.height/size.width;
            CGFloat imgHeight = kScreenWidth * imgAspecio;
            weak_self.holidayAdViewHeightConstraint.constant = imgHeight;
            getHolidayAdHeightBlock(imgHeight);
        }];

    }else {
        self.holidayAdViewHeightConstraint.constant = 0;
        self.specialPartToHolidayTopEdgeConstraint.constant = 0;
        getHolidayAdHeightBlock(0);
