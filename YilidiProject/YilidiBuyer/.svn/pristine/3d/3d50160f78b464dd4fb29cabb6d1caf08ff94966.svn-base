...
-(void)_initRightItem {
    ...
    [rightItemButton setTitleColor:KSelectedTitleColor forState:UIControlStateNormal];
...
- (NSString *)_varyFyInput {
    ...
    if (isEmpty(self.provinceCode) || isEmpty(self.countyCode) || isEmpty(self.cityCode)|| isEmpty(self.communityId)) {
        errorStr = @"亲，请选择正确的城市和地区";
    }
    return errorStr;
}
...
- (void)_alertDeleteAdress:(void(^)(bool delete))backBlock {
    [self showSimplyAlertWithTitle:@"提示" message:@"是否确定删除地址？" sureAction:^(UIAlertAction *action) {
        backBlock(YES);
    } cancelAction:^(UIAlertAction *action) {
        backBlock(NO);
    }];
...
- (IBAction)deleteSaveAdressAction:(id)sender {
    if (self.operatorAddressType == EditAdressType) {
        [self _alertDeleteAdress:^(bool delete) {
            if (delete) {
