...
- (void)setPhotosDes {
    ...
    for (id object in self.photos) {
        if ([object isKindOfClass:[PYPhoto class]]) {
            PYPhoto *photo = (PYPhoto *)object;
            if (i >= _detailDesArray.count) {
                break;
            }
            NSString *des = _detailDesArray[i];
            photo.detailDesStr = des;
        }
        i ++;
    }
}
...
- (void)setDetailDesArray:(NSArray<NSString *> *)detailDesArray {
    if (detailDesArray.count == 0) return;
    
    _detailDesArray = detailDesArray;
...
//    [self.addImageButton removeFromSuperview];
    ...
    NSLog(@"imageheightCell1:%f",size.height);
...
//    [self.addImageButton removeFromSuperview];
    ...
    CGFloat width = size.width + self.py_x > PYScreenW ? PYScreenW - self.py_x : size.width;
    self.py_size = CGSizeMake(width, size.height);
    NSLog(@"imageheightCell2:%f",size.height);
    // 刷新
    [self layoutSubviews];
}
