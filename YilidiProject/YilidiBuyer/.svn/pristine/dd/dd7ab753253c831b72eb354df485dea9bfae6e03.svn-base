...
- (void)requestLogin {
    ...
    NSDictionary *loginParam = @{@"mobile":self.phoneField.text,
                                 @"code":self.codeField.text,
                                 @"type":KTypeCodeLogin};
    WEAK_SELF
    [AFNHttpRequestOPManager postWithParameters:loginParam subUrl:kUrl_Login block:^(NSDictionary *result, NSError *error) {
        if (error.code==1) {
...
- (IBAction)countDownCode:(JKCountDownButton *)sender {
    if (!endBtn) {
        [self showSimplyAlertWithTitle:@"提示" message:@"请输入有效的11位手机号码" sureAction:^(UIAlertAction *action) {
...
-(BOOL)isCheckContent:(NSString *)text{
    ...
    if (_editorStatus==TextEditorPhoneFieldTag) {
        if (text.length == 11) {
            ...
            [_loginButton setTitleColor:KTextColor forState:UIControlStateNormal];
            [_loginButton setBackgroundColor:KLineColor];
