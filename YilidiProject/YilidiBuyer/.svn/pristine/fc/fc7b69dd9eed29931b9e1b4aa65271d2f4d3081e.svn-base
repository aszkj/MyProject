...
- (void)setUpTabBarViewController {
    ...
    //分类
    DLClassificationVC *classificationVC = [[DLClassificationVC alloc]init];
    classificationVC.hidesBottomBarWhenPushed = YES;
    
    DLBaseNavController *classificationNav = [[DLBaseNavController alloc]initWithRootViewController:classificationVC];
    ...
    self.viewControllers = @[homePageNav,classificationNav,cartVCNav,meVCNav];
...
- (void)_customTabBarView
{
    ...
    NSArray *tabbarItemImgNames = @[@"首页",@"分类",@"购物车",@"我的"];
    NSArray *tabbarItemSelecteImgNames = @[@"首页选中",@"分类选中",@"购物车选中",@"我的选中"];
    
    NSArray *tabbarItemTitles = @[@"首页",@"分类",@"购物车",@"我的"];
    // 3. 设置选中视图
...
- (BOOL)_needLoginAtTabbarPage:(NSInteger)pageIndex {
    
    if (pageIndex == MainPageIndex || pageIndex == GoodsClassPageIndex) {
        return NO;
    }else {
        return YES;
    }
...
- (void)_enterLoginPage {
    DLLoginVC *loginVC = [[DLLoginVC alloc] init];
    [self.currentShowVC.navigationController pushViewController:loginVC animated:YES];
}

- (void)_enterShopCartPage {
    DLShopCarVC *shopCartPage = [[DLShopCarVC alloc] init];
    [self.currentShowVC.navigationController pushViewController:shopCartPage animated:YES];
}
...
- (void)click:(UIButton *)button
{
    ...
    if ([self _needLoginAtTabbarPage:clickIndex]) {
        if (!UserIsLogin) {
...
-(UIButton *)shopCartMessageButton {
    ...
    if (!_shopCartMessageButton) {
        ...
        _shopCartMessageButton.backgroundColor = KCOLOR_PROJECT_RED;
...
- (UIViewController *)currentShowVC {
    //这里不能用懒加载，因为这里点击切换tabbar的时候，currentVC是不断变化的
