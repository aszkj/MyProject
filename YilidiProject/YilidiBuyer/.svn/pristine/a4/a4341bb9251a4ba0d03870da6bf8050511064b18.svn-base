...
- (void)_init{
    ...
    //未安装了微信隐藏
    if ([[kUserDefaults objectForKey:IsInstallationWechat]isEqualToString:@"NO"]){
    
         self.wechatAndQQbgView.hidden = YES;
    }else{
    
        self.wechatAndQQbgView.hidden = NO;
    }
...
- (void)_leaveLoginPage {
    if ([[PushMessageManager sharedManager] hasMessageNeedToJump]) {
        [[PushMessageManager sharedManager] jumpToMessageDestinationPage];
        [PushMessageManager sharedManager].messageJumpThroughLogin = YES;
    }else {
...
- (void)_enterMain {
    ...
    DDLogWarn(@"usertoken:%@",usertoken);
    DDLogWarn(@"userclientId:%@",userclientId);
...
- (void)getVerification {
    ...
    [self showLoadingHub];
    NSDictionary *requestParam = @{@"mobile":self.phoneFiled.text,
...
- (void)requestLogin{
    ...
    NSDictionary *loginParam = @{@"mobile":self.accountStr,
                                 @"code":self.passWordStr,
                                 @"type":KTypeLoginPassword};
    
    [AFNHttpRequestOPManager postWithParameters:loginParam subUrl:kUrl_Login block:^(NSDictionary *resultDic, NSError *error) {
        if (error.code==1) {
...
- (IBAction)wechatLogin:(id)sender {
    ...
    if ([WXApi isWXAppInstalled]){
        ...
        [WXApi sendReq:req];
         ...
         [Util ShowAlertWithOnlyMessage:@"未安装微信客户端，无法进行登录"];
    }
...
- (IBAction)qqLogin:(id)sender {
    ...
    if ([TencentOAuth iphoneQQInstalled]) {
        ...
        [Util ShowAlertWithOnlyMessage:@"未安装QQ客户端，无法进行登录"];
...
-(BOOL)isCheckContent:(NSString *)text{
    ...
    if (_editorStatus==TextEditorPhoneFieldTag) {
        if (text.length == 11) {
            ...
            [_registerBtn setTitleColor:KTextColor forState:UIControlStateNormal];
            [_registerBtn setBackgroundColor:KLineColor];
