...
@property (nonatomic) TakeMoneyViewModel *viewModel;
@end
...
- (void)bindViewModel {
    ...
    RAC(self.yuerText,placeholder) = [RACObserve(self.viewModel, balance) map:^id(NSNumber *value) {
        return [NSString stringWithFormat:@"可提现余额：%@元",[NSString decoratedString:value.stringValue]];
    }];
...
- (void)errorHandle:(NSError *)error {
    ...
    if ([error.domain isEqualToString:@"cash_passwor_not_exist"]) {
        [self.navigationController pushViewController:[[XKUpdatePasswdController alloc] init] animated:YES];
    }
}
...
- (TakeMoneyViewModel *)viewModel {
    if (_viewModel == nil) {
        _viewModel = [[TakeMoneyViewModel alloc] init];
        @weakify(self)
...
- (void)updateOnClassInjection {
    ...
    [self.dataCenter requestOperatorInfoFromModel:info successBlk:^(NSArray *responseData) {
        DDLogInfo(@"XKO_BaseInfoRequestInfo successBlk:%@",responseData);
       
    } failBlk:^(NSError *error) {
        DDLogError(@"XKO_BaseInfoRequestInfo error");
    }];
    
    XKO_MembersManagerRequestInfo *member = [[XKO_MembersManagerRequestInfo alloc] init];
    member.api_pageNum = @(1);
    member.api_pageSize = @(8);
    [self.dataCenter requestOperatorMemberListFromModel:member successBlk:^(NSArray *responseData) {
        DDLogInfo(@"XKO_MembersManagerRequestInfo successBlk:%@",responseData);
        
    } failBlk:^(NSError *error) {
        DDLogError(@"XKO_MembersManagerRequestInfo error");
    }];
}
