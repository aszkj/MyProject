...
-(void)requestVeryCodeForRegister:(BOOL)isForRegister failed:(Failed)failed{
...
- (void)beginLoginWithSuccess:(Sucess)sucess failed:(Failed)failed
{
    ...
    NSLog(@"用户名 :%@ 密码 : %@",_userName,_userPasswd);
...
//        failed(error.localizedDescription);
//    }];*/
    PasswordForgetOperatorUpdateRequest *request = [[PasswordForgetOperatorUpdateRequest alloc] init:@""];
    request.api_mobile = _userName;
    request.api_verifyCode = _veryfyCode;
    request.api_password = _userPasswd;
    [vapManager passwordForgetOperatorUpdate:request success:^(AFHTTPRequestOperation *operation, PasswordForgetOperatorUpdateResponse *response) {
            if (response.errorCode.integerValue > 0) {
                failed(response.subMsg);
            }else {
                sucess(@"findPasswdSuccess");
