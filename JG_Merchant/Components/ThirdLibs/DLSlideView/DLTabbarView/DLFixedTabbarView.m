...
- (void)switchingFrom:(NSInteger)fromIndex to:(NSInteger)toIndex percent:(float)percent{
    ...
    [UIView animateWithDuration:0.26 delay:0.15 options:UIViewAnimationOptionCurveEaseIn animations:^{
...
- (void)setSelectedIndex:(NSInteger)selectedIndex{
    if (_selectedIndex != selectedIndex) {
        ...
        if (kSelectedCount) {
            [UIView animateWithDuration:0.26 delay:0.15 options:UIViewAnimationOptionCurveLinear animations:^{
                trackView_.frame = CGRectMake(trackX, trackView_.frame.origin.y, CGRectGetWidth(trackView_.bounds), CGRectGetHeight(trackView_.bounds));
