...
@interface IncomeCardViewController () <UITableViewDelegate,UITableViewDataSource>
@property (nonatomic) UITableView *tableView;
@property (nonatomic) NSArray *dataSource;
@end

@implementation IncomeCardViewController
...
- (void)showMoneyListWithIndex:(NSInteger)index {
    ...
    if (index == 0) {
        ...
        statisticsVC.statisticsType = StatisticsOffLine;
        statisticsVC.title = @"线下服务收入统计";
        
        [self.navigationController pushViewController:statisticsVC animated:YES];
        
    } else if (index == 1) {
        
        XKJHTheDetailController *theDetailVC = [[XKJHTheDetailController alloc] init];
        theDetailVC.theDetailType = OfflineSwipeCardDetailType;
        theDetailVC.title = @"线下服务收入明细";
...
- (NSArray *)dataSource {
    if (_dataSource == nil) {
        _dataSource = @[@[@"Consumption statistics",@"线下刷卡收入统计"],@[@"Detail List",@"线下刷卡明细列表"]];
    }
    return _dataSource;
}
