...
@interface ConsumptionSubRunViewController () <UITableViewDelegate,UITableViewDataSource>
@property (nonatomic) UITableView *tableView;
@property (nonatomic) NSArray *dataSource;
@end

@implementation ConsumptionSubRunViewController
...
- (void)showMoneyListWithIndex:(NSInteger)index {
    ...
    if (index == 0) {
        DDLogDebug(@"显示消费分润明细");
        XKJHStatisticsController *statisticsController = [[XKJHStatisticsController alloc] init];
        statisticsController.title = @"消费返润统计";
        statisticsController.statisticsType = StatisticsConsume;
        
        [self.navigationController pushViewController:statisticsController animated:YES];

    } else if (index == 1) {
        VC = [[SubRunCountViewController alloc] init];
...
- (NSArray *)dataSource {
    if (_dataSource == nil) {
        _dataSource = @[@[@"Consumption statistics",@"消费返润统计"],@[@"Detail List",@"返润明细列表"]];
    }
    return _dataSource;
}
