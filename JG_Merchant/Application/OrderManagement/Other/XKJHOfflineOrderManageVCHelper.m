//
//  XKJHOfflineOrderManageVCHelper.m
//  Merchants_JingGang
//
...
@interface XKJHOfflineOrderManageVCHelper() {
...
//当前刷新的是那个表
@property (nonatomic, strong)XKJHOfflineOrderManageTableView *currentFreshTableView;
@end

@implementation XKJHOfflineOrderManageVCHelper
...
-(void)_loadTableInView:(UIView *)vcRootView tableCount:(NSInteger)tableCount {
    ...
    for (int i=0; i<tableCount; i++) {
        XKJHOfflineOrderManageTableView *tableView = [[XKJHOfflineOrderManageTableView alloc] initWithFrame:CGRectMake(0, kTopBarHeight+kTableToTopbarGap, kScreenWidth, kScreenHeight-kTopBarHeight-kNavBarAndStatusBarHeight-kTableToTopbarGap) style:UITableViewStylePlain];
...
-(void)clickTopbarAtIndex:(NSInteger)clickedTopBarIndex {
    ...
    //找到之前显示的view，将其隐藏
    XKJHOfflineOrderManageTableView *lastDisplayedTableView = (XKJHOfflineOrderManageTableView *)[Util getDisplayedViewAtViewArrs:_contentTableArr];
    lastDisplayedTableView.hidden = YES;
    
    //显示选中的index对应的contentView
    XKJHOfflineOrderManageTableView *willDisplayedTableView = (XKJHOfflineOrderManageTableView *)[_contentTableArr objectAtIndex:clickedTopBarIndex];
