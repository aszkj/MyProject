...
-(void)_initGoodsSearchResultCollectionView {
    ...
    WEAK_SELF
    self.classificationCollection.nodataAlertTitle = @"抱歉，没有相关商品";
    [self.classificationCollection configureCollectionCellNibName:@"DLHomeGoodsVerticalCell" configureCollectionCellData:^(UICollectionView *collectionView, id collectionModel, UICollectionViewCell *collectionCell) {
...
//
//    NSDictionary * dic = @{@"lakeBlue":UIColorFromRGB(0xf0f0f0),@"navyBlue":UIColorFromRGB(0x2271bf),@"lightBlue":UIColorFromRGB(0x48acdd),@"red":UIColorFromRGB(0xf88afb),@"yellow":UIColorFromRGB(0xF6C71A),@"green":UIColorFromRGB(0xa6e172)};
//    
//    self.classificationCollection.backgroundColor = dic[colorStr];
//    
//    
//    return dic[colorStr];
//}
...
- (void)_performAddShopCartAnimationUsingImgView:(UIImageView *)animationImageView isAdd:(BOOL)isAdd{
    ...
    self.classificationCollection.scrollEnabled = NO;
    WEAK_SELF
    [self.animatePerformer performProjectDefaultAddShopCartAnimationForAnimationGoodsImgView:animationImageView animateEndPoint: CGPointMake(kScreenWidth-34-50/2, kScreenHeight-58 - 50/2) animateDidEndBlock:^{
        weak_self.classificationCollection.scrollEnabled = YES;
        [kNotification postNotificationName:KNotificationShopCartBadgeValueNeedChange object:@(isAdd)];
    }];
}
...
- (void)requestGoodsData {
    ...
    [AFNHttpRequestOPManager postWithParameters:requestParam subUrl:kUrl_GetClassZoneProducts block:^(NSDictionary *resultDic, NSError *error) {
        ...
        [weak_self.classificationCollection configureTableAfterRequestData:transferedArr];
