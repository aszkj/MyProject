//
//  DLAddEditAdressVC.h
//  YilidiBuyer
//
//  Created by yld on 16/5/17.
//  Copyright © 2016年 yld. All rights reserved.
//

#import "DLBuyerBaseController.h"
#import "ProjectRelativEmerator.h"
@interface DLAddEditAdressVC : DLBuyerBaseController

@property (nonatomic,assign)OperatorAddressType operatorAddressType;

@property (nonatomic,assign)ComeToAdresModuleType comeToAdresModuleType;

@property (nonatomic,copy)NSString *shippingAddressId;

@property (nonatomic,assign)BOOL isComeForQuilocate;


@end
