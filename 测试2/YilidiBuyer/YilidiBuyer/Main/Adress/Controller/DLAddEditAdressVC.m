...
- (void)_initAddAdressDefaultUIInfo {
    [self.consigneeAddressButton setTitle:[CommunityDataManager sharedManager].communityModel.countyName forState:UIControlStateNormal];
    [self.cityButton setTitle:[CommunityDataManager sharedManager].communityModel.cityName forState:UIControlStateNormal];
}
...
-(void)_initRightItem {
    ...
    [rightItemButton setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
...
- (void)_initSomeCanBeChangedAdressInfoWithAdressModel:(AdressModel *)adressModel {
    ...
    if (self.operatorAddressType == EditAdressType) {
        self.communityId = adressModel.commnunityId;
        self.provinceCode = adressModel.provinceCode;
        self.cityCode = adressModel.cityCode;
        self.countyCode = adressModel.townShipCode;
    }else {
...
- (void)_asignUiWithData:(AdressModel *)model{
    ...
    [self.consigneeAddressButton setTitle:model.commnunityName forState:UIControlStateNormal];
...
- (void)_notifyShopCartPageWithResultDic:(NSDictionary *)resultDic isDeleteAdress:(BOOL)isDeleteAdress{
    AdressModel *adressModel = [[AdressModel alloc] init];
    if (!isDeleteAdress) {
        NSString *adressId = [resultDic[EntityKey][@"addressId"] stringValue];
        adressModel.consigneAdressId = adressId;
        adressModel.commnunityId = self.communityId;
        adressModel.consigneePersonName = resultDic[EntityKey][@"consigneeName"];
        adressModel.consigneePersonPhoneNumber = resultDic[EntityKey][@"phoneNo"];
        adressModel.consigneePersonalDetailAdress = resultDic[EntityKey][@"addressDetail"];
        StoreModel *storeInfo = [[StoreModel alloc] initWithDefaultDataDic:resultDic[EntityKey][@"storeInfo"]];
        adressModel.storeInfo = storeInfo;
    }else {
...
- (void)_assignSelectedAddressInfo:(CommunityModel *)selectAdressModel {
    ...
    self.consigneeBuidingRoomNumberTextField.text = selectAdressModel.communityAdressDetail ;
...
- (void)_showAlertWhenSelectNotInTheShipRangeWithSelecteCommunityModel:(CommunityModel *)adressModel {
    WEAK_SELF
    [self showSimplyAlertWithTitle:@"提示" message:@"对不起，你所选的地址不在配送范围内，是否选择这个地址" sureAction:^(UIAlertAction *action) {
