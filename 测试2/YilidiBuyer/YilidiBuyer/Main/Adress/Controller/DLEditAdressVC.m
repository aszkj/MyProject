...
#import "DLNearByAdressSearchVC.h"
...
#pragma mark ------------------------Private-------------------------
- (void)_asignUiWithData:(DLAdressListModel *)model{
...
- (void)_comtoSelectedNeartyAdressPage {
    DLNearByAdressSearchVC *adressSearchVC = [[DLNearByAdressSearchVC alloc] init];
...
- (void)_requestEditAdress{
    ...
    [self.editAdressViewModel requestEditAdressWithAdressParam:[self _getEditAdressParam] backBlcok:^(NSDictionary *backDic, NSError *error) {
        [self hideLoadingHub];
        [self performSelector:@selector(_popToMakeSureVC) withObject:nil afterDelay:0.2];
    }];
}
