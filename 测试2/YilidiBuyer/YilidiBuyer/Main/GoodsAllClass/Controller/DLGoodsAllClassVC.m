...
-(void)_initInfo {
    ...
    if (!isEmpty(self.infoDic)) {
        self.enterFromParentClassCode = self.infoDic[kLinkDataKeyGoodsClass];
    }
...
- (void)_initgoodsListTableView {
    ...
    self.goodsListTableView.nodataAlertTitle = @"暂无商品";
...
- (void)_initGoodsFirstLevelCategaryTableView {
    ...
    [self.goodsFirstCategaryTableView configurecellNibName:@"DLCommunityGoodsFirstLevelCategaryCell" configurecellData:^(UITableView *tableView,id cellModel, UITableViewCell *cell) {
        ...
        firstLevelCategaryCell.titleLabel.textColor = (model.selected ? KSelectedBgColor : [UIColor darkTextColor]);
        firstLevelCategaryCell.lineView.backgroundColor = (model.selected ? KSelectedBgColor : UIColorFromRGB(0xE3E5E5));
...
- (CommonCategaryModel *)_findSelectedClassCodeModleWhenFirstEnterAllClassPageWithFirstClassCodeModels:(NSArray *)firstClassCodeModels {
    
    if ([self.enterFromParentClassCode isEqualToString:TOP_CLASS_CODE]) {
        return firstClassCodeModels.firstObject;
    }else {
        __block CommonCategaryModel *classCodeModel = nil;
        [firstClassCodeModels enumerateObjectsUsingBlock:^(CommonCategaryModel *obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([obj.categaryId isEqualToString:self.enterFromParentClassCode]) {
                classCodeModel = obj;
                * stop = YES;
            }
        }];
        if (isEmpty(classCodeModel)) {
            classCodeModel = firstClassCodeModels.firstObject;
        }
...
//    CommonCategaryModel *model = tranferedArr.firstObject;
    CommonCategaryModel *model = [self _findSelectedClassCodeModleWhenFirstEnterAllClassPageWithFirstClassCodeModels:tranferedArr];
    model.selected = YES;
...
- (void)_requestGoodsData{
    self.requestParam = @{@"classCode":self.requestGoodsListComponsedCategaryId,
...
//        self.goodsListTableView.dataLogicModule.totalPage = [result[EntityKey][kTotalPages] integerValue];
...
- (IBAction)saleValumeSortAction:(id)sender {
    ...
    if (self.saleVolumeButton.selected) {
        ...
        self.priceSortDownArrowButton.selected = NO;
        self.priceSortUpArrowButton.selected = NO;
        _sortByNumber = kGoodsSortTypeNumberFromHighToLow;
