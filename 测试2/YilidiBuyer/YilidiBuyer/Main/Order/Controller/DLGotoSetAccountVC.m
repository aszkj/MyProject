...
#define SelectTicketButtonTitleColor UIColorFromRGB(0xff3a30)
#define DonotUseTicketButtonTitleColor UIColorFromRGB(0x333333)
#define LeftAvaliableTicketsButtonTitleColor UIColorFromRGB(0xff3a30)
#define NoneAvaliableTicketsButtonTitleColor UIColorFromRGB(0x8b8b8b)
...
- (void)_configureSelectTicketButton:(UIButton *)ticketButton
    ...
    if (ticketAvaliableCount) {//有可用的抵用券，不能选他，它就为不使用抵用券
        ticketButtonColor = LeftAvaliableTicketsButtonTitleColor;
        ticketEabled = YES;
        ticketTitle = jFormat(@"%ld张可用",ticketAvaliableCount);
    }else {//无可用
        ticketButtonColor = NoneAvaliableTicketsButtonTitleColor;
        ticketEabled = NO;
        ticketTitle = @"无可用";
    }
...
- (void)_configureSelectTicketButtonsIfJustSupportTicketSingleSelect {
    ...
    if (totalAmountMaxTicketType == CouponTicket) {
        ...
        if (self.avaliablePledgeCount) {//有可用的抵用券，不能选他，它就为不使用抵用券
            ...
            pledgeTicketButtonColor = NoneAvaliableTicketsButtonTitleColor;
        ...
        if (self.avaliableCouponCount) {//有可用的抵用券，不能选他，它就为不使用抵用券
            ...
            couponTicketButtonColor = NoneAvaliableTicketsButtonTitleColor;
        ...
        [self _setButton:self.selectCouponValueButton eabled:NO title:@"无可用" titleColor:NoneAvaliableTicketsButtonTitleColor];
        [self _setButton:self.selectPledgeValueButton eabled:NO title:@"无可用" titleColor:NoneAvaliableTicketsButtonTitleColor];
    }
    ...
    if (totalAmountMaxTicketType == UnknownTicket) {
        selecteButtonColor = NoneAvaliableTicketsButtonTitleColor;
        selecteButtonEabled = NO;
        [self.currentSelectButton setTitle:jFormat(@"无可用") forState:UIControlStateNormal];
    }else {
        //这里标题在另外地方设置了
        selecteButtonColor = SelectTicketButtonTitleColor;
...
- (void)_configureCurrentSelectTicketButtonAfterSelectTickes:(NSArray *)selectTickets useTicket:(BOOL)useTicket {
    if (selectTickets.count > 0) {//选择了券
        //设置数量标题
        [self _setTotalAmountForCurrentSelectTicketButton];
        [self.currentSelectButton setTitleColor:SelectTicketButtonTitleColor forState:UIControlStateNormal];
    }else {//没有选择券
        ...
        if (!useTicket) {//选择了不使用券
            ...
            selectTicketButtonColor = LeftAvaliableTicketsButtonTitleColor;
        }
