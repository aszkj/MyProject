...
-(void)_initSegmented {
    ...
    self.topSegmentedView.textColor = UIColorFromRGB(0x333333);
    self.topSegmentedView.selectedTextColor = KSelectedBgColor;
    self.topSegmentedView.selectionIndicatorColor = KSelectedBgColor;
...
- (void)_statusTableView{
    ...
    [self.statusTableView configurecellNibName:@"DLOrderStatusCell" cellDataSource:self.statusArr configurecellData:^(UITableView *tableView, id cellModel, UITableViewCell *cell) {
        ...
        if (model.modelAtIndexPath.row==0) {
            statusCell.orderStatus.textColor = UIColorFromRGB(0x333333);
            statusCell.lineView.hidden=YES;
            statusCell.circleButton.selected=YES;
        }else{
            statusCell.orderStatus.textColor  = UIColorFromRGB(0xbcbcbc);
...
- (void)_orderFeeInfoData:(NSDictionary *)dic {
    ...
    _detailsFooterView.payableAmountLabel.text = [NSString stringWithFormat:@"￥%.2f",[dic[@"payableAmount"] floatValue]/1000];
