...
- (void)getVerification {
    [self showLoadingHub];
    WEAK_SELF
    [AFNHttpRequestOPManager requestVaryCodeForPhoneNumber:self.phoneField.text varyCodeType:KTypeBingdingPhone block:^(id result, NSError *error) {
        if (error.code==1) {
...
- (void)_login{
    ...
    WEAK_SELF
    [AFNHttpRequestOPManager bingdingPhoneLogin:dic block:^(id result, NSError *error) {
        if (error.code==1) {
...
-(BOOL)isCheckContent:(NSString *)text{
    ...
    if (_editorStatus==TextEditorPhoneFieldTag) {
        if (text.length == 11) {
            ...
            [_loginBtn setTitleColor:UIColorFromRGB(0x8b8b8b) forState:UIControlStateNormal];
            [_loginBtn setBackgroundColor:UIColorFromRGB(0xdcdcdc)];
