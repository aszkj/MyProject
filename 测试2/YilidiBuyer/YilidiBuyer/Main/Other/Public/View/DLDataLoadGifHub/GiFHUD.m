...
//        [self.layer setBackgroundColor:[[UIColor colorWithWhite:0 alpha:0.5] CGColor]];
        ...
        self.showLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, self.bounds.size.height-20, self.bounds.size.width, 20)];
        self.showLabel.text = @"正在拼命加载中..";
        self.showLabel.font = [UIFont systemFontOfSize:12.0];
        self.showLabel.textColor = [UIColor darkGrayColor];
...
+ (void)dismiss {
    if (![[self instance] shown])
        return;
    UIView *overlayView = (UIView *)[[self instance] overlay];
