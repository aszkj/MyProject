...
/**
 * 
 * @Description:TODO(远程文件删除客户端)
 * @author: chenlian
 * @date: 2015年11月19日 下午3:19:34
 * 
 */
public class RemoteFileDeleteClient {

    private Logger logger = Logger.getLogger(RemoteFileDeleteClient.class.getName());
    ...
    private static final String SUCCESS_TIPS = "文件远程删除成功";

    private static final String DELIMITER = "$_$_@_@_$_$!@#$%^&*()";

    public RemoteFileDeleteClient(String host, int port, String remoteFilePath) {
        this.host = host;
        this.port = port;
    ...
    private class HeartBeatHandler extends ChannelInboundHandlerAdapter {
        ...
        public void userEventTriggered(ChannelHandlerContext ctx, Object evt) throws Exception {
            ...
            if (evt instanceof IdleStateEvent) {
                ...
                if (event.state().equals(IdleStateEvent.WRITER_IDLE_STATE_EVENT.state())) {
                    ...
                    logger.info(currentTime + " ：发送心跳Ping信息到远程文件删除服务端 ");
                ...
                if (event.state().equals(IdleStateEvent.READER_IDLE_STATE_EVENT.state())) {
                    ...
                    throw new IllegalStateException("与远程文件删除服务端连接超时.....");
                }
            }
        }
        ...
        public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) throws Exception {
            logger.error("远程文件删除客户端出现系统异常", cause);
            ctx.close();
            throw new IllegalStateException("远程文件删除客户端出现系统异常", cause);
        }
    ...
    private class FileDuplicateHandler extends ChannelInboundHandlerAdapter {
        ...
        public void channelActive(ChannelHandlerContext ctx) throws Exception {
            logger.info("==========================================================> 发送删除文件信息到远程文件删除服务端 .....");
        ...
        public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
            ...
            if ("Pong".equals(messageFromServer)) {
                ...
                SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                String currentTime = sdf.format(date);
                logger.info(currentTime + " ：接收到远程文件删除服务端的心跳应答Pong信息 ");
            } else {
                logger.info("==========================================================> " + messageFromServer);
                // 对于这种远程文件删除场景，其实是不需要保持客户端与服务端的长连接的，因其操作不会太频繁，每次需要远程删除时，进行一次短连接，处理完毕后关闭链路即可。
                // 成功远程删除后，关闭Channel。
