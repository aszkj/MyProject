 ...
 * 
 * @author: chenb
 * @date: 2016年7月6日 下午12:03:40
 * 
 */
@Controller("loginWarehouseController")
@RequestMapping(value = "/warehouse")
public class LoginWarehouseController extends WarehouseBaseController {
    ...
    public MsgBean login(HttpServletRequest request, @RequestBody UserBaseDto useBaseDto) {
        try {
            UserDto userDto = userService.loginValidate(useBaseDto.getUserName(),
                    SystemContext.UserDomain.CUSTOMERTYPE_SELLER, SystemContext.UserDomain.LOGINTYPE_PASSWORD,
                    useBaseDto.getPassword());
            StoreProfileDto storeProfileDto = storeProfileService.loadByStoreId(userDto.getCustomerId(), null);
            if (ObjectUtils.isNullOrEmpty(storeProfileDto)) {
                return super.encapsulateMsgBean(MsgBean.MsgCode.FAILURE, "店铺不存在");
            }
            if (!SystemContext.UserDomain.STORETYPE_MICROWAREHOUSE.equals(storeProfileDto.getStoreType())) {
                return super.encapsulateMsgBean(MsgBean.MsgCode.FAILURE, "亲,您还不是微仓用户");
            }
            ...
            if (null == llDto) {
                ...
                loginLogDto.setCustomerType(SystemContext.UserDomain.CUSTOMERTYPE_SELLER);
            ...
            session.setAttribute(USERID_WAREHOUSE_SESSIONKEY, userDto.getId());
            session.setAttribute(STOREID_WAREHOUSE_SESSIONKEY, userDto.getCustomerId());
            session.setAttribute(STORENAME_WAREHOUSE_SESSIONKEY, userDto.getUserName());
            session.setAttribute(MASTER_USERID_WAREHOUSE_SESSIONKEY, userDto.getMasterUserId());
            // 获取用户的菜单权限
    ...
    private List<PermissionDto> getUserPermissionMenus() {
        try {
            String customerType = SystemContext.UserDomain.CUSTOMERTYPE_SELLER;
