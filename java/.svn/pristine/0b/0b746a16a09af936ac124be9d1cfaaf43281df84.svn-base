 ...
 * 
 */
public class SecKillOrderReportExport extends AbstractExportReport {

    protected Logger logger = Logger.getLogger(this.getClass());

    private IOrderService orderServiceHessian;

    public SecKillOrderReportExport() {
        super.reportIndividualRelativePath = "/seckillorder";
    }
        ...
        try {
            if (!ObjectUtils.isNullOrEmpty(dataList)) {
                for (int i = 0; i < dataList.size(); i++) {
                    ...
                    addCell(row, 4, dto.getStatusName());
                    addCell(row, 5, dto.getDeliveryModeName());
                    addCell(row, 6, dto.getConsigneeUserName());
                    addCell(row, 7, dto.getPhoneNo());
                    addCell(row, 8, dto.getAddressDetail());
                    addCell(row, 9, dto.getCreateTime());
                    addCell(row, 10, dto.getPayTime() == null ? "---" : dto.getPayTime());
                    addCell(row, 11, ArithUtils.div(dto.getTotalAmount(), StringUtils.BASE_AMOUNT, 3));
                    addCell(row, 12, dto.getPayTypeName());
                    rowIndex++;
                }
            }
            return rowIndex;
