...
/**
 * @Description:TODO(买家用户账本(欧币)交易明细管理导出报表) 
 * @author:	llp
 * @date:	2015年12月3日 下午5:16:38 
 *
 */
public class UserAccountDetailReportExport extends AbstractExportReport {
	...
	public UserAccountDetailReportExport() {
		...
		try {
			if (!ObjectUtils.isNullOrEmpty(dataList)) {
				for (AccountDetailDto accountDetailDto : (List<AccountDetailDto>) dataList) {
					...
					addCell(row, 1, accountDetailDto.getUserName());
					addCell(row, 2, accountDetailDto.getCreateTime());
					addCell(row, 3, ArithUtils.div(ArithUtils.convertLongTodouble(accountDetailDto.getCashAccountAmount()),
									baseMount, 3));
					addCell(row, 4, systemBasicDataInfoUtils.getSystemDictName(
									SystemContext.UserDomain.DictType.ACCOUNTDETAILTYPE.getValue(),
									accountDetailDto.getDetailType()));
					addCell(row, 5, accountDetailDto.getSerialNo());
					addCell(row, 6, systemBasicDataInfoUtils.getSystemDictName(
									SystemContext.UserDomain.DictType.ACCOUNTCHANGETYPE.getValue(),
	...
	public static List<Map<String, String>> withdrawReportHeader() {
		...
		map = new HashMap<String, String>();
		map.put("header", "欧币数量(个/元)");
		map.put("width", "25");
		headers.add(map);
		
		map = new HashMap<String, String>();
		map.put("header", "收入/支出");
		map.put("width", "20");
		headers.add(map);
