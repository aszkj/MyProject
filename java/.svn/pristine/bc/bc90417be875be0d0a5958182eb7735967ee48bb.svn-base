...
iTsai.form = {
    ...
    _serializeGroups : function(groups) {
        ...
        for ( var i = groups.length - 1; i >= 0; i--) {
            ...
            if(fieldsetArray.indexOf(key) == -1){
            	fieldsetArray[j] = key;
                j++;
            }
        }
        for(var t = 0; t < fieldsetArray.length ; t++){
            ...
            for ( var i = groups.length - 1; i >= 0; i--) {
                ...
                if(fieldsetArray[t] == key){
                    ...
                    inputArray = inputArray.concat(array);
                }
            }
            json[fieldsetArray[t]] = this._serializeInputs(inputArray);
           for (var k = 0; k < inputArray.length ; k++){
               // 添加序列化标记
        	   $(inputArray[k]).addClass('_isSerialized');
           }
        }
        return json;
    },
     ...
     * 2).子容器（也可以没有）：必须包括属性fieldset="XXX" span标签，里面包含输入标签和子容器。<br>
    ...
    serialize : function(frm) {
        ...
        var groups = frm.find('span[fieldset]');
        ...
        if(inputs){
        	for(var i = 0;i<inputs.length;i++){
        		...
        		inputsNew.push(inputs[i]);
        	}
        }
        var json = this._serializeInputs(inputsNew);
     ...
     * 
     * @param{$()} frm jQuery表单对象（或其它容器标签对象，如：div、span）
    ...
    deserialize : function(frm, json) {
        ...
        for ( var key in groups) {
            var json = groups[key];
            var span = frm.find('span[fieldset="' + key + '"]');
            if (!span.length) {
                continue;
            }
            var inputs = _filterInputs(span);
            if (!inputs.length) {
                continue;
            }
            for ( var k in json) {
