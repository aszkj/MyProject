...
/**
 * @Description:TODO(买家用户在线交易:(订单在线支付和退款)日志管理导出报表) 
 * @author:	llp
 * @date:	2015年12月3日 下午5:16:38 
 *
 */
public class UserPayLogReportExport extends AbstractExportReport {
		...
		try {
			if (!ObjectUtils.isNullOrEmpty(dataList)) {
				for (OnlinePayDetailDto onlinePayDetailDto : (List<OnlinePayDetailDto>) dataList) {
					...
					addCell(row, 1, onlinePayDetailDto.getUserName());
					addCell(row, 2, onlinePayDetailDto.getSerialNo());
					addCell(row, 3, systemBasicDataInfoUtils.getSystemDictName(
									SystemContext.OrderDomain.DictType.SALEORDERPAYPLATFORM.getValue(),
									onlinePayDetailDto.getPayPlatform()));
					addCell(row, 4,	ArithUtils.div(ArithUtils.convertLongTodouble(onlinePayDetailDto.getPayAmount()),
									baseMount, 3));
					addCell(row, 5, systemBasicDataInfoUtils.getSystemDictName(
					...
					addCell(row, 6, systemBasicDataInfoUtils.getSystemDictName(
									SystemContext.UserDomain.DictType.REFUNDTYPE.getValue(),
									onlinePayDetailDto.getRefundType()));
