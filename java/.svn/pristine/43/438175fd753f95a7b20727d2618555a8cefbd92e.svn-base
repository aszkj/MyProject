 ...
 */
public class LoginSmsMessageModel extends SmsMessageModel {
     ...
     */
    private static final String CONTENT_TEMPLATE_NAME_PROPERTIES_KEY = "sms.login.template";
    ...
    /**
     * 短信内容模版中定义的变量（登陆验证码有效时间）， 短信内容模版中定义的变量在不同的短信模版里可能不同，有可能没有，有可能有多个，需根据具体短信灵活定义
     */
    private static final String CONTENT_TEMPLATE_VARIABLE_LOGIN_VALIDTIME = "loginValidTime";

    public LoginSmsMessageModel(List<String> toUserMobileList, String... variables) {
        ...
        if (StringUtils.isEmpty(variables[1])) {
            throw new IllegalArgumentException("模版中定义的变量：loginValidTime 不能为空！");
        }
        this.toUserMobileList = toUserMobileList;
        this.smsProvideType = SystemContext.SystemDomain.SMSNOTIFYPROVIDETYPE_HUAXIN;
        this.smsMsgType = SystemContext.SystemDomain.SMSNOTIFYMSGTYPE_LOGIN;
        ...
        this.contentVariablesMap.put(CONTENT_TEMPLATE_VARIABLE_LOGIN_VALIDTIME, variables[1]);
    }
