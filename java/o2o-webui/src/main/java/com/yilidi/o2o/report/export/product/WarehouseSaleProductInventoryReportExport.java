 ...
 */
public class WarehouseSaleProductInventoryReportExport extends AbstractExportReport {
     ...
     * 构造方法初始化
     */
    public WarehouseSaleProductInventoryReportExport() {
        super.reportIndividualRelativePath = "/warehouse/saleproduct/inventory";
    }
        ...
        try {
            if (!ObjectUtils.isNullOrEmpty(dataList)) {
                for (int i = 0; i < dataList.size(); i++) {
                    ...
                    addCell(row, 6, saleProductDto.getRemainCount());
                    addCell(row, 7, saleProductDto.getOrderCount());
                    rowIndex++;
                }
            }
            return rowIndex;
