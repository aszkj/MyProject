 ...
 * 
 * @author: heyong
 * @date: 2015年12月2日 上午10:47:27
 * 
 */
public class OrderStatisticsReportExport extends AbstractExportReport {

	protected Logger logger = Logger.getLogger(this.getClass());

	private IOrderService orderServiceHessian;

	public OrderStatisticsReportExport() {
		super.reportIndividualRelativePath = "/orderstatistics";
	}
	...
	protected Long obtainDataCount(Object searchArgument) throws ReportException {
		try {
			...
			return orderServiceHessian.getCountsForExportOrderStatistics(saleOrderStatisticsQuery);
	...
	protected List<?> obtainDataList(Object searchArgument, Long startLineNum, Integer pageSize) throws ReportException {
		try {
			SaleOrderStatisticsQuery saleOrderStatisticsQuery = (SaleOrderStatisticsQuery) searchArgument;
			return orderServiceHessian.listDataForExportOrderStatistics(saleOrderStatisticsQuery, startLineNum, pageSize);
		} catch (OrderServiceException e) {
	...
	protected int generateHead(Sheet sheet, int rowIndex) throws ReportException {
		try {
			List<Map<String, String>> headers = orderStatisticsReportHeader();
			String mainTitle = "用户销售统计报表";
		...
		try {
			if (!ObjectUtils.isNullOrEmpty(dataList)) {
				for (int i = 0; i < dataList.size(); i++) {
					...
					addCell(row, 0, i + 1);
					addCell(row, 1, dto.getUserName());
					addCell(row, 2, dto.getTotalOrderCount());
					addCell(row, 3, ArithUtils.div(dto.getTotalAmount(), StringUtils.BASE_AMOUNT, 3));
					if (!ObjectUtils.isNullOrEmpty(dto.getTotalOrderCountFromSuccess())) {
						addCell(row, 4, dto.getTotalOrderCountFromSuccess());
					} else {
						addCell(row, 4, 0);
					}
					if (!ObjectUtils.isNullOrEmpty(dto.getTotalAmountFromSuccess())) {
						addCell(row, 5, ArithUtils.div(dto.getTotalAmountFromSuccess(), StringUtils.BASE_AMOUNT, 3));
					} else {
						addCell(row, 5, 0);
					}
					if (!ObjectUtils.isNullOrEmpty(dto.getTotalOrderCountFromCancel())) {
						addCell(row, 6, dto.getTotalOrderCountFromCancel());
					} else {
						addCell(row, 6, 0);
					}
					if (!ObjectUtils.isNullOrEmpty(dto.getTotalAmountFromCancel())) {
						addCell(row, 7, ArithUtils.div(dto.getTotalAmountFromCancel(), StringUtils.BASE_AMOUNT, 3));
					} else {
						addCell(row, 7, 0);
					}
					rowIndex++;
	 ...
	 */
	public static List<Map<String, String>> orderStatisticsReportHeader() {
