...
/**
 * @Description:TODO(买家用户账本管理导出报表) 
 * @author:	llp
 * @date:	2015年12月3日 下午5:16:38 
 *
 */
public class UserAccountReportExport extends AbstractExportReport {
	...
	public UserAccountReportExport() {
	...
	protected Long obtainDataCount(Object searchArgument) throws ReportException {
		try {
			...
			return accountServiceHessian.getCountsForExportUserAccount(query);
	...
	protected List<?> obtainDataList(Object searchArgument, Long startLineNum, Integer pageSize) throws ReportException {
		try {
			CustomerBalanceQuery  query = (CustomerBalanceQuery) searchArgument;
			return accountServiceHessian.listDataForExportUserAccount(query, startLineNum, pageSize);
		} catch (UserServiceException e) {
		...
		try {
			if (!ObjectUtils.isNullOrEmpty(dataList)) {
				for (AccountDto accountDto : (List<AccountDto>) dataList) {
					...
					addCell(row, 1, accountDto.getUserName());
					addCell(row, 2, accountDto.getTelPhone());
					addCell(row, 3,
							ArithUtils.div(ArithUtils.convertLongTodouble(accountDto.getCurrentBalance()), baseMount, 3));
					rowIndex++;
				}
			}
			return rowIndex;
