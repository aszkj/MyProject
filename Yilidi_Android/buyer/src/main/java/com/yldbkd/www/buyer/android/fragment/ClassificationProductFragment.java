 ...
 * @更新描述
 */
public class ClassificationProductFragment extends BaseFragment {
    ...
    //刷新数据
    private List<SaleProduct> classityProductLists = new ArrayList<>();
    private HttpBack<Page<SaleProduct>> classityProducHttpBack;
    ...
    public void initBundle() {
        ...
        classCode = bundle.getString(Constants.BundleName.CLASS_CODE, "");
    }

    @Override
    public int setLayoutId() {
        return R.layout.classity_product_fragment;
    }
    ...
    public void initView(View view) {
        ...
        productGridView = (PullToRefreshGridView) view.findViewById(R.id.lv_classity_product);
    ...
    private void initAdapter() {
        productAdapter = new ZoneProductAdapter(classityProductLists, getActivity(), 1, productHandler);
    ...
    public void onResume() {
        super.onResume();
        CartUtils.calculateProductNum(classityProductLists);
    ...
    public void initHttpBack() {
        classityProducHttpBack = new HttpBack<Page<SaleProduct>>(getBaseActivity()) {
            @Override
            public void onSuccess(Page<SaleProduct> data) {
                ...
                if (data.getPageNum() <= 1) {
                    classityProductLists.clear();
                }
                if (data.getList() != null) {
                    classityProductLists.addAll(data.getList());
                }
                if (classityProductLists.size() == 0) {
                    mNodataRL.setVisibility(View.VISIBLE);
                    productGridView.setVisibility(View.GONE);
                } else {
    ...
    private void request(Integer mPageNum) {
        ...
        RetrofitUtils.getInstance(true).searchProducts(ParamUtils.getParam(map), classityProducHttpBack);
    }

    private static class ProductHandler extends Handler {
        WeakReference<ClassificationProductFragment> fragmentWeakReference;

        public ProductHandler(ClassificationProductFragment fragment) {
            fragmentWeakReference = new WeakReference<>(fragment);
        }

        @Override
        public void dispatchMessage(Message msg) {
            ClassificationProductFragment fragment = fragmentWeakReference.get();
            if (fragment == null) {
                return;
            }
            SaleProduct product = fragment.classityProductLists.get(msg.arg2);
            switch (msg.what) {
                ...
                case Constants.HandlerCode.CART_SUCCESS:
                    CartUtils.calculateProductNum(fragment.classityProductLists);
    ...
    static class RefreshHandler extends Handler {
        private WeakReference<ClassificationProductFragment> orderWeakReference;

        public RefreshHandler(ClassificationProductFragment fragment) {
            this.orderWeakReference = new WeakReference<>(fragment);
        }

        @Override
        public void dispatchMessage(Message msg) {
            ClassificationProductFragment fragment = orderWeakReference.get();
            if (fragment == null) {
                return;
            }
