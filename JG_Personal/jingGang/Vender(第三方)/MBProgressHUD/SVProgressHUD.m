...
{
    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"提示" message:alert delegate:nil cancelButtonTitle:nil otherButtonTitles:@"确认", nil];
    [alertView show];
...
- (void)setStatus:(NSString *)string {
    ...
    if(stringWidth > hudWidth)
    {
        ...
        stringLabelWidth = hudWidth;
    }
    
    int nLine = 1;
    
    if (hudWidth > __MainScreen_Width - 100)
    {
        hudWidth = __MainScreen_Width - 80;
        stringLabelWidth = __MainScreen_Width - 100;
        nLine = stringWidth / stringLabelWidth + 1;
    }
    
    _hudView.bounds = CGRectMake(0, 0, hudWidth, 100 + (nLine - 1) * 20);
    
    self.imageView.center = CGPointMake(CGRectGetWidth(_hudView.bounds)/2, 36);
    
    self.stringLabel.hidden = NO;
    self.stringLabel.text = string;
    self.stringLabel.frame = CGRectMake((hudWidth - stringLabelWidth) / 2, 66, stringLabelWidth, nLine * 20);
