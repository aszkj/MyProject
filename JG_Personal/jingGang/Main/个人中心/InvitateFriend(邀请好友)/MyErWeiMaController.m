...
-(void)_usersInfoRequest {
    ...
    [_vapManager usersCustomerSearch:request success:^(AFHTTPRequestOperation *operation, UsersCustomerSearchResponse *response) {
        ...
        if (!customer.name || [customer.name isEqualToString:@"(null)"]||[customer.name isEqualToString:@""]) {
            weak_self.strName = customer.nickName;
        }else{
...
-(void)showActionSheet {
    ...
    ALActionSheetView *actionSheetView = [ALActionSheetView showActionSheetWithTitle:nil cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles:@[@"我的邀请码",@"填写邀请码",@"邀请明细",@"分享设置"] handler:^(ALActionSheetView *actionSheetView, NSInteger buttonIndex) {
